...
void GetSectionHeadersInfo(SECTION_HEADER* p)
{
	...
	TCHAR* strformat=(TCHAR* )TEXT("_IMAGE_SECTION_HEADER{"
					...
					"\n\tCharacteristics: 0x%08x\n}");
	char section_name[9];
	strcpy_s(section_name,(const char*)p->Name);
	section_name[8]='\0';
	//DbgPrintf(strformat,
			//p->Name,
			//p->Misc,
...
void CheckRelocationInfo(LPBYTE lpbuffer)
{
	if (lpbuffer == NULL)
	{
		printf("Get a NULL.\n");
		...
		if (virtual_addr != 0 && block_size != 0)
		{
			...
			for (j = 0; j < block_item_num; j++)
			{
				...
				printf("%d  %08X(rva)  %08X(foa)  %d(type)\n", j,
					(int)(block_item + virtual_addr),
					(int)RVAtoFOA((__int64)(block_item + virtual_addr),
						(LPBYTE)lpbuffer), block_item_type);
			}
			system("pause>nul");
		}
		else {
