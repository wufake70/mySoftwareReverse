    ...
    _In_ int       nCmdShow)
{
    ...
    if (IsPackingEXE(hInstance))
    {
        ...
        lpSrcImgBase = VirtualAllocEx(pI.hProcess,
            (LPVOID)(ullSrcImgBase+0x4000*i),
            ullSrcImgSize,
            MEM_COMMIT | MEM_RESERVE,
            PAGE_EXECUTE_READWRITE);
        while (lpSrcImgBase == NULL)
        {
            i++;
            lpSrcImgBase = VirtualAllocEx(pI.hProcess,
                (LPVOID)(ullSrcImgBase + 0x4000 * i),
        ...
        if ((ULONG64)lpSrcImgBase != ullSrcImgBase)
        {
            ...
            if (RepairByRelocTable(lpDecryptedData, ullSrcImgBase) == FALSE)
            {
        ...
        //MessageBox(0,aa,0,0);
        //if (UnmapViewOfSection(pI.hProcess, (PVOID)dwShellImage) == FALSE)
        //{
        //    TerminateProcess(pI.hProcess, 0);
        //    CloseHandle(pI.hProcess);
        //    CloseHandle(pI.hThread);
        ...
        {
            // MessageBox(0, TEXT("VirtualAlloc Fail. Need to relocate."), 0, 0);
            if (RepairByRelocTable(lpDecryptedData, dwSrcImgBase) == FALSE)
            {
                MessageBox(0, TEXT("没有导入表"), 0, 0);
                TerminateProcess(pI.hProcess, 0);
                CloseHandle(pI.hProcess);
