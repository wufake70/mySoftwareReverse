...
NTSTATUS DriverUnload(PDRIVER_OBJECT pDriver)
{
	...
	KdPrintEx((77,0,"DriverEntry Unloading...\n"));
...
NTSTATUS GeneralDispatch(PDEVICE_OBJECT pDevice, PIRP pIrp)
{
	...
	KdPrintEx((77,0,"General Diapatch\n"));
...
NTSTATUS ReadComp(PDEVICE_OBJECT pDevice, PIRP pIrp, PVOID Context)
{
	...
	if (NT_SUCCESS(pIrp->IoStatus.Status))
	{
		...
		// SystemBuffer是一个数组,Information记录着大小
		keyNumber = (ULONG)(pIrp->IoStatus.Information / sizeof(PKEYBOARD_INPUT_DATA));
		//for (ULONG i = 0; i < keyNumber; i++)
		for (ULONG i = keyNumber-1; i < keyNumber; i++)
		{
			if (PCScanfCode[myData->MakeCode] == 0) break;;
			if (myData->Flags != KEY_MAKE) break;	// 只记录按下
			KdPrintEx((77,0,"scanfcode:0x%x\tchar:%s\t%s\n",
				myData->MakeCode,
				PCScanfCode[myData->MakeCode],
...
NTSTATUS ReadDispatch(PDEVICE_OBJECT pDevice, PIRP pIrp)
{
		...
		// 设置IRP完成回调函数,IoCompleteRequest触发
...
NTSTATUS PowerDispatch(PDEVICE_OBJECT pDevice, PIRP pIrp)
{
	...
	IoSkipCurrentIrpStackLocation(pIrp);	// Irp->CurrentLocation++;
...
NTSTATUS AttachDevice(PDRIVER_OBJECT pDriver, PUNICODE_STRING RegPatch)
{
	...
	{
		KdPrintEx((77,0,"Open KeyBoard Driver Failed\n"));
		return status;
	}
	else
	{
	...
	while (targetDevice)
	{
		...
		if (!NT_SUCCESS(status))
		{
			KdPrintEx((77,0,"Create New FilterDevice Failed\n"));
		...
		// filterDevice->Flags &= DO_DEVICE_INITIALIZING;	// 去掉设备初始化
		// 遍历下一个设备
		targetDevice = targetDevice->NextDevice;
	}
	KdPrintEx((77,0,"Create And Attach Finshed...\n"));
	return status;
}
...
{
	memset(&PCScanfCode[60], 0, 0xff - 60);
	ULONG i;
	NTSTATUS status = STATUS_SUCCESS;
