...
int main(int argc, char* argv[])
{
	...
	*/
	...
	if(file_buffer)
	{
		while(1)
		{
			printf("Input <rva[0]|foa[1]> <offset>: ");
			scanf("%x %x",&option,&offset);
			if(option==0)
			{
				printf("FOA: 0x%I64X\n",RVAtoFOA(offset,file_buffer));
			}else{
				printf("FOA: 0x%I64X\n",FOAtoRVA(offset,file_buffer));
			}
		}
	}else{
	...
	*/
	...
	// move export table & move relocation table & repairByRelocaTable
	/*
	BYTE log_code=0;
	char pepath[100];
	char new_pepath[100];
	...
	if(GetPE_StdPtr(file_buffer)->Machine==0x14c)
	{
		...
		old_img_base=p_optpe->ImageBase;
		printf("The ImageBase of current PE: %X.\n",old_img_base);
	}else{
		printf("Not a x86 PE.\n");
		return 0;
	}
	printf("Input a value to modify the ImageBase: ");
	scanf("%x",&img_base);
	p_optpe->ImageBase=img_base;  // modify ImageBase;

	new_section_head=(section_head*)malloc(40);
	if(new_section_head)
	{
		log_code++;
		if(repairByRelocaTable(file_buffer,old_img_base))
		{
			log_code++;	
	...
	switch(log_code)
	{
		...
		case 1:
			free(new_section_head);
			printf("repairByRelocaTable() UNsuccessful.\n");
			break;
		case 2:
			free(new_section_head);
			free(new_section_buffer);
