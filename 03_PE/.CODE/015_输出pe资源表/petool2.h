...
void CheckResourceTable(char* file_buffer)
{
	...
	{
		p_resource_entry=&((resource_entry*)(&p_resource_type[1]))[i];

		// unicode or id
		if(p_resource_entry->NameIsString)
		{
			pstr=(str_unicode*)((char*)p_resource_type+p_resource_entry->NameOffset);
			memset(name,0,0x30);
...
void CheckResourceInfo(char* file_buffer,size_t index)
{
	...
	resource_entry* p_resource_entry = &((resource_entry*)(&p_resource_type[1]))[index];
	resource_entry* p_resource_serial_entry=NULL;
	resource_entry* p_resource_charset_entry=NULL;
	...
	if(p_resource_entry->DataIsDirectory)
	{
		...
		{
			p_resource_serial_entry = &((resource_entry*)(&p_resource_serial[1]))[i];
			// unicode or id
			if(p_resource_serial_entry->NameIsString)
			{
				pstr = (str_unicode*)((char*)p_resource_type+p_resource_serial_entry->NameOffset);
				memset(name,0,0x30);
				memcpy(name,pstr->NameString,pstr->Length*(sizeof wchar_t));

				// 代码页(字符集)
				p_resource_charset = (resource_directory*)((char*)p_resource_type+p_resource_serial_entry->OffsetToDirectory);
				p_resource_charset_entry = (resource_entry*)&p_resource_charset[1];
