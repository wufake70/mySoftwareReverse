...
__int64 FOAtoRVA(__int64 rva,char* p)
{
	if(p==NULL)
	{
		printf("get a NULL.\n");
	...
	{
		// vc++6.0 __int64类型变量作比较存在问题
		if((size_t)roa>=(size_t)GetPE_sectionPtr(p,i)->PointerToRawData && (size_t)roa<(size_t)(GetPE_sectionPtr(p,i)->PointerToRawData+GetPE_sectionPtr(p,i)->SizeOfRawData))
		{
			virtual_address=GetPE_sectionPtr(p,i)->VirtualAddress;
			file_address=GetPE_sectionPtr(p,i)->PointerToRawData;
...
int Add_NewSection(char** fbuffer,size_t file_size,section_head* new_section_head,char* new_section_buffer,int new_section_size)
{
	...
	if(headers_size-(int)p_section+(int)fbuffer[0]>80)
	{
		...
		if(i==80)
		{
			...
			if(GetPE_StdPtr(fbuffer[0])->Machine==0x14c)
			{
				GetPE_OptPtr(fbuffer[0])->SizeOfImage=img_size+AutoAlign(new_section_size,section_align);
			}else{
				GetPE_Opt64Ptr(fbuffer[0])->SizeOfImage=img_size+AutoAlign(new_section_size,section_align);
			}
...
int moveExportTable(char* file_buffer,char** new_section,size_t* new_section_size)
{
	...
	i=0;
	while((size_t)i<name_total_size)
	{
...
int repairByRelocaTable(char* file_buffer,DWORD old_img_base)
{
	...
	p_relocation=(relocation_directory*)(&file_buffer[RVAtoFOA((__int64)p_directory->VirtualAddress,file_buffer)]);
	...
	while(1)//virtual_addr!=0&&block_size!=0
	{
		...
		if(virtual_addr!=0&&block_size!=0)
		{
			...
			for(j=0;j<block_item_num;j++)
			{
				...
				{
					((DWORD*)(&file_buffer[(int)RVAtoFOA((__int64)(block_item+virtual_addr),file_buffer)]))[0]+=img_delta;
				}
			}
		}else{
			break;
